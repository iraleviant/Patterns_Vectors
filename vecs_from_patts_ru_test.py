...
def add_patt_instance(words, start, patt_index, patterns_trie, cws_clean_order, co_mat, word_vocab, context_vocab, ofh):
    ...
    elif  not bool(re.match(r'[^\W0-9a-z]+$', words[start])) or not(len(words[start])) :
        return
    ...
    elif (bool(re.search(r'[^\W0-9a-z]+$', words[start]))) and (words[start] in cws_clean_order ) and (CW_SYMBOL in patterns_trie.node ) :
...
def main():
    ...
    input_files="/home/ira/Google_Drive/IraTechnion/PhD/corpus_ru/wiki_ru"
    input_files="/home/ira/Google_Drive/IraTechnion/PhD/corpus_ru/russian_test"
    #input_files="/home/ira/Google_Drive/IraTechnion/PhD/python/py_patts/clean_ru.txt"

    patterns_input_file='final_symm_patt_russian1.txt' #"selected_patterns_py.txt"

    
    context_vocab_file="context_vocab_out_all_rus1.txt"#file mapping context strings to 
    context_pairs_output_file ="context_file_out_all_rus1.txt"
    dic_file='cws_dictionary_all_rus1.dat'
    dic_file_order='cws_dictionary_allpats_order_rus1.dat'
    word_vocabulary_output_file='word_vocab_python_allpats__rus1.dat'
    mat_file='all_pats_python_mat_rus1.npz'

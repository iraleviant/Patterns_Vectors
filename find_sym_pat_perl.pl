...
#use constant WORD_RE => qr/^[a-z]++$/;

# Split token (to separate words and punctuation)
#use constant SPLIT_TOKEN => "[ \t]++"; #\t-tab, 
use constant SPLIT_TOKEN => "[ \t]++|\\b"; #\t-tab, \b-word border, splits on an end of a word
...
sub gen_HFW_dict($$$$$) {
	...
	while (my $line = $ifh->getline() and (not defined $top_n_lines or $line_ctr < $top_n_lines)) {
		...
		foreach my $w (@words) {
			next unless $w =~ WORD_RE;  #qr/^[a-z]++$/;    #elsif ($w !~ WORD_RE or not exists $cws->{$w}) {
	...
	write_vocab(\%vocab, "vocab_perl.txt");
	
	print "Found ".scalar(keys %vocab). " keys in vocab.\n";  
	$ifh->close();
	...
	for (my $i = 0 ; $i < $end ; ++$i) {
		...
		foreach my $k ($i .. $i + MIN_PATT_LENGTH-2) {
			if ($types[$k] == NO_WORD) {
				...
				$i = $k; #should be $i = $k its an error , was $i = $k+1
				last;
			} elsif ($types[$k] == CW) {
...
sub add_edges_func($$$) {
	...
	if (exists $dict->{$str}) {
		...
		#my $line_ctr = 0; #remove  for debug
	}
}

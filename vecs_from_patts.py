...
def add_patt_instance(words, start, patt_index, patterns_trie, cws_clean_order, co_mat, word_vocab, context_vocab, ofh):
    ...
    elif  not bool(re.match(r'^[a-z_]+$', words[start])) or not(len(words[start])) :
        return
    ...
    elif (bool(re.search(r'^[a-z_]+$', words[start]))) and (words[start] in cws_clean_order ) and (CW_SYMBOL in patterns_trie.node ) :
...
def main():
    ...
    context_vocab_file="context_vocab_out_python_2prase.txt"#file mapping context strings to 
    context_pairs_output_file ="context_file_out_python_2phrase.txt"
    dic_file='cws_dictionary_all_pats_python_2phrase.dat'
    dic_file_order='cws_dictionary_allpats_python_order_2phrase.dat'
    word_vocabulary_output_file='word_vocab_python_allpats_python_2phrase.dat'
    mat_file='all_pats_python_mat_2phrase.npz'

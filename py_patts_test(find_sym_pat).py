...
class Dict:
    def __init__(self, dic,pattern_edges, cws_list):
        self.dict = dic
...
def main():
    ...
    #files="/home/ira/Google_Drive/IraTechnion/PhD/corpus_copy/webbase_all_clean.txt,/home/ira/Google_Drive/IraTechnion/PhD/corpus_copy/clean_wiki_new.txt,/home/ira/Google_Drive/IraTechnion/PhD/corpus_copy/billion_word_clean.txt,/home/ira/Google_Drive/IraTechnion/PhD/corpus_copy/news_2012_clean,/home/ira/Google_Drive/IraTechnion/PhD/corpus_copy/news_2013_clean"
    #files="/home/ira.leviant@st.technion.ac.il/clean_corpus/webbase_all_clean.txt,/home/ira.leviant@st.technion.ac.il/clean_corpus/clean_wiki_new.txt,/home/ira.leviant@st.technion.ac.il/clean_corpus/billion_word_clean.txt,/home/ira.leviant@st.technion.ac.il/clean_corpus/news_2012_clean,/home/ira.leviant@st.technion.ac.il/clean_corpus/news_2013_clean"

    #infile="/home/ira/Google_Drive/IraTechnion/PhD/corpus_copy/webbase_all_clean.txt"
     
    infile="/home/ira/Google_Drive/IraTechnion/PhD/corpus/english_test"  # Input file of plain text to compute set of SPs.
    files='/home/ira/Google_Drive/IraTechnion/PhD/corpus/english_test'
    
    #infile="/home/ira.leviant@st.technion.ac.il/clean_corpus/webbase_all_clean.txt"
    #infile="/home/ira.leviant@st.technion.ac.il/clean_corpus/english_test"
    #files= "/home/ira.leviant@st.technion.ac.il/clean_corpus/english_test"
    
    
    
    n_hfws= 100#1000 ##100 # Number of high frequency words (HFWs) that can serve as pattern elements.
    n_cws=10000   # Number of content words (CWs) for computing the M measure (for efficiency)
    outfile="final_selected_sym_merge_patts_python.txt"  # Output file for selected SPs.
    m_thr = 0.02#0.05   # Threshold of M measure for selecting SPs, The only real important parameter is m_thr
    max_pattern_length = 5 ##7   # Maximum pattern length to consider.
    min_num_of_edges_per_pattern = 3#5000#3   # Minimum of edge types for a pattern to be considered a candidate. (5000)
    ...
    write_vocab(pattern_candidates, all_patts_candidates_file)
    ...
    write_sps(sym_patts_file, selected_patterns)
...
def extract_patterns(line, lc, max_pattern_length, hfw_dict, cws, func, dic):
    line=line.rstrip()
    if lc:
        line=lc(line) #lower case line
    ...
    # First, take the first MIN_PATT_LENGTH-1 words that must appear in each pattern. #the first two words 
        ...
        for k in range(i+MIN_PATT_LENGTH-1, end2):
            ...
            if len(p1.cws_list)==2 and has_hfw:
                ...
                if func==1:
                    add_pattern_instance_func(dic, st, p1.cws_list)
                else:
                    add_edges_func(dic, st, p1.cws_list)   
                #next line here  $func->($dict, $str, \@cws);
                
        i=i+1


def add_pattern_instance_func(dic, st, extra_param):
    if st not in dic:
        dic[st]=1
    else:
        dic[st]+=1  
    
def add_edges_func(dic, st, extra_param):
    if st in dic.keys():
        if not extra_param[0] in dic[st]: ### errorrr# unless (exists $dict->{$str}->{$extra_param->[0]}) {
            dic[st][extra_param[0]]={}
            
        if extra_param[1] not in dic[st][extra_param[0]]:
            dic[st][extra_param[0]][extra_param[1]]=1  ###????
        else:
            dic[st][extra_param[0]][extra_param[1]]+=1
...
def write_sps(of, select_sps):
    ...
    for k in  sotred_sps:
        #ifh.write(str(k)+'\n')# tha patt and its m ('CW and CW', 0.666)
        ifh.write(str(k[0])+'\n') #only the patt itself 'CW and CW'
    ifh.close()
    
def usage(options, mandatory, help): #ERROR
    for k in mandatory:
        if k=="ARRAY":
            for k2 in k:
...
def getopt_gen_message(options, mandatory):  ####ERROR

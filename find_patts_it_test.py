...
def main():
    ...
    files='/home/ira/Dropbox (Technion Dropbox)/corpus_it/clean_it'
    infile='/home/ira/Dropbox (Technion Dropbox)/corpus_it/clean_it'
    
    files='/home/ira/Dropbox (Technion Dropbox)/corpus_it/italian_test.dat'
    infile='/home/ira/Dropbox (Technion Dropbox)/corpus_it/italian_test.dat'
    
    patts_candidates_file="patts_candidates_it.dat"
    selected_patts_file="selected_patts_it.dat "
    final_outfile_patts="final_sym_merged_patt_it.dat"  # Output file for selected SPs.
    
    
    n_hfws= 10##100 # Number of high frequency words (HFWs) that can serve as pattern elements.
    ...
    min_num_of_edges_per_pattern = 2#200#3   # Minimum of edge types for a pattern to be considered a candidate. (5000)
    n_pattern_candidates = 5000   # Number of patterns to considered (the N most frequent patterns)
    top_n_lines = 2000000   # Use only the top N lines for computing vocabulary and list of pattern candidates.
    min_edge_frequency =1#3 # 3   # Minimal frequency for edge to be considered in the 
                                    # graph construction.
    merge_sps= True  # Optional: merge patterns that are a longer version of                                     #             another selected SP. 
    ...
    write_vocab(pattern_candidates, patts_candidates_file)
    ...
    write_sps(selected_patts_file, selected_patterns)
    ...
    print ("Writing selected patterns to ",final_outfile_patts )
    write_sps(final_outfile_patts, selected_patterns)
...
def  gen_HFW_dict(infile, n_hfws, n_cws, lc, top_n_lines):
    ...
    for line in ifh:
        ...
        if top_n_lines!=True or line_ctr < top_n_lines:
            ...
            for w in words:
                if not bool(re.match(r'[^\W0-9]+$', w)): #only words that include a-z or _ or $ included special word dont go through like confédération
...
def extract_patterns(line, lc, max_pattern_length, hfw_dict, cws, func, dict):
    ...
    for w in words:
        ...
        elif not bool(re.match(r'[^\W0-9]+$', w)) or  (w not in cws.keys()) :  #elsif ($w !~ WORD_RE or not exists $cws->{$w}) errpr

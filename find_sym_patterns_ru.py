...
def main():
    ...
    infile="/home/ira/Google_Drive/IraTechnion/PhD/corpus_ru/wiki_ru"  # Input file of plain text to compute set of SPs.
    files='/home/ira/Google_Drive/IraTechnion/PhD/corpus_ru/wiki_ru'
    
    #infile="/home/ira/Google_Drive/IraTechnion/PhD/corpus_ru/russian_test"  # Input file of plain text to compute set of SPs.
    #files='/home/ira/Google_Drive/IraTechnion/PhD/corpus_ru/russian_test'

    
    n_hfws= 1000#1000 ##100 # Number of high frequency words (HFWs) that can serve as pattern elements.
    n_cws=10000   # Number of content words (CWs) for computing the M measure (for efficiency)
    m_thr =  0.05#0.02#0.05   # Threshold of M measure for selecting SPs, The only real important parameter is m_thr
    max_pattern_length = 5 ##7   # Maximum pattern length to consider.
    min_num_of_edges_per_pattern =1000 #3#5000#3   # Minimum of edge types for a pattern to be considered a candidate. (5000)
    ...
    merge_sps= True  # Optional: merge patterns that are a longer version of 
                                    #             another selected SP. 
    lc = True   # Convert text to lower case

    outfile="final_selected_sym_merge_patts_python_russ.txt"  # Output file for selected SPs.
    all_patts_candidates_file="all_patterns__candidates_python_russ.txt"
    sym_patts_file="selected_symmetric_patts_python_russ.txt"
...
def  gen_HFW_dict(infile, n_hfws, n_cws, lc, top_n_lines):
    ...
    try:
        ifh = codecs.open(infile, 'r', encoding="Latin-1")
    except:
    ...
    for line in ifh:
        ...
        if top_n_lines!=True or line_ctr < top_n_lines:
            ...
            for w in words:
                #if not bool(re.match(r'^[а-я]+$', w)): #only words that include a-z or _ or $ included special word dont go through like confédération
    ...
    for k in sorted_words: # k is a tuple ("the", 464)
        n_words+=1
        if  n_words < n_hfws: #if the number of words read so far (sorted) is smaller than the requierd number of hwf
...
def extract_patterns(line, lc, max_pattern_length, hfw_dict, cws, func, dic):
    ...
    for w in words:
        ...
        elif not bool(re.match(r'[^\W0-9]+$', w)) or  (w not in cws.keys()) : 
...
def read_pattern_edges(files, hfw_dict, cws, pattern_candidates, min_num_of_edges_per_pattern, max_pattern_length, lc):
    ...
    for filek in ifs:
        ...
        with open(filek, 'r', encoding="Latin-1") as ifh:
            for line in ifh:
                line=line.rstrip()
                line_ctr+=1
...
def write_sps(of, select_sps):
    ...
    try:
        ifh = codecs.open(of, 'w', encoding="Latin-1")
    except:
...
def write_vocab(dic, output_file):
    ...
    try:
        ofh = codecs.open(output_file, 'w', encoding="Latin-1")
    except:
